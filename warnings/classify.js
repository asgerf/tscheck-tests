...
inputs.forEach(function(input) {
	...
	var interfaceLines = input.lines.exclude(/\[IMPL\]/).exclude(/\[INIT\]/).exclude(/\[PLATFORM\]/)
	...
	assert.equal(input.lines.length, impl + snapshot + interfce, "Some warning were underclassified or overclassified in " + input.file)

	report(input.file, [highFix, lowFix, highUnfix, lowUnfix, impl, init, platform])
})
